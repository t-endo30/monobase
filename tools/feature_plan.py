...
def draft(pl, site):
    ...
    rows = []
    for a in picked:
        good_for_items = (a.get("good_for") or {}).get("items") or []
        good_for_title = (good_for_items[0].get("title") or "") if good_for_items else ""
        rows.append([
            f'<a href="{a["slug"]}.html">{a.get("list_title") or a["title"]}</a>',
            (a.get("verdict_title") or "").replace("結論：", "") or "—",
            (good_for_title[:40] + "…") if good_for_title else "—",
        ])

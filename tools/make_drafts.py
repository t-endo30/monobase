...
# 商品名の先頭に付くモールの名前。店が検索に当てるために付けている
# だけで、商品名ではない（「楽天 薄型ペットシーツ …」という題名に
# なっていた）。先頭にあるときだけ落とす。「Amazonセール買い時
# カレンダー」のように、主題の一部になっている言い方は残す。
MALL_HEAD = re.compile(
    r"^\s*(楽天市場|楽天|Yahoo!?ショッピング|ヤフー|Amazon|アマゾン)"
    r"(?=[\s　【\[（(]|$)")

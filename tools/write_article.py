...
def build_prompt(a, site, prompt_md, fetch_official=True):
    ...
    return "\n".join([
        ...
        "「〇日間使用した」は禁止）。「メーカー仕様から見ると」「利用者の声では」"
        ...
        "・voices に stars（星の数）を入れない。voices は利用者の声の"
...
def facts_block(a):
    ...
    if not facts:
        return ("\n【仕様について】メーカー公式で裏を取った仕様は渡されていません。"
                ...
                "・数値・機能の有無を断定しない\n"
                "・そのうえで、記事は書き上げる\n"
                "公式仕様が無くても書けることを書きます。利用者の声から読み取れる傾向と"

...
def has_brand(name):
    """商品名に、商品を特定できる語があるか。
       ...
       「ポータブルシャワー」「LED投光器コンセント式」のように、品目と
       その飾りしか入っていない題名がある。この語で検索すれば、同じ語を
       含む商品はいくらでも返り、含有率は 1.00 になる。しかし同じ商品
       とは限らない。実際、LED投光器の記事に別物のセンサーライトが、
       34インチモニターの記事に別ブランドのモニターが当たった。

       日本語の題名は語が続けて書かれる（「34インチウルトラワイド
       モニター」で1語）。区切って判定すると素通りするので、
       品目と飾りの言葉を、文字列として順に消していく。
       消したあとに何か残れば、それがブランド名か型番。"""
    rest = norm(name)
    for w in sorted(GENERIC | GENERIC_MOD_WORDS, key=len, reverse=True):
        rest = rest.replace(w.lower(), " ")
    # 「34インチ」「100W」のような、数字と単位の組
    rest = re.sub(r"[0-9]+\s*(インチ|型|畳|w|v|l|ml|mm|cm|k|hz|°|度)?", " ", rest)
    rest = re.sub(r"[^0-9a-zぁ-んァ-ヶ一-龠]+", " ", rest)
    left = [t for t in rest.split() if len(t) >= 2]
    return bool(left)
    ...
    for a in targets:
        ...
        if not jan and not model_tokens(name):
            ...
            if picked:
                ...
                # 品目しか書かれていない題名では、検索しても同じ商品か
                # 確かめようがない。ここで止める。
                if not has_brand(name):
                    print(f"\n・{a.get('slug')}：題名が品目だけで商品を特定"
                          f"できないため飛ばします（{name}）")

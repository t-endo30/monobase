...
# 品目に付く飾り。これも商品を特定しない。
GENERIC_MOD = re.compile(
    r"^(ポータブル|コンパクト|ワイヤレス|コードレス|電動|手動|折りたたみ|"
    r"角度調整|収納付き|スチーム式|気化式|超音波式|据置|卓上|小型|大型|"
    r"薄型|軽量|静音|省エネ|工事不要|屋外|屋内|防水|防塵|LED|DC|AC|USB|"
    r"[0-9０-９]+(インチ|型|W|V|L|ml|畳)?|コンセント式|人感センサー|"
    r"ウルトラワイド|フロントオープン|導光板|ソーラー|スマート)$")
...
def has_brand(name):
    """商品名に、商品を特定できる語があるか。
       ...
       品目とその飾りを取り除いて、何か残るかどうかで見る。
       残ればブランド名か型番なので、検索して照合する意味がある。"""
    left = []
    for t in tokens(name):
        if not t:
            continue
        if t in GENERIC or GENERIC_MOD.match(t):
            continue
        # 品目の言葉を含んでいるだけの語（「ポータブルシャワー」など）
        if any(g in t for g in GENERIC if len(g) >= 2):
            rest = t
            for g in sorted(GENERIC, key=len, reverse=True):
                rest = rest.replace(g, "")
            rest = GENERIC_MOD.sub("", rest)
            if len(rest) <= 1:
                continue
        left.append(t)
    return bool(left)

...
A8の広告リンクは、次の2つで取り込みます。

```bash
# ① A8の「広告リンク」ページで tools/a8-collect.js をコンソールに貼る
#    → そのページのコードが全部クリップボードに入る（codes.txt に保存）
# ② 提携中プログラムのCSVと突き合わせて、名前とカテゴリーを自動で付ける
python3 tools/import_a8.py --csv programs.csv --codes codes.txt          # 確認
python3 tools/import_a8.py --csv programs.csv --codes codes.txt --apply  # 反映
```

コードの `mid=` の先頭15桁が、CSVの「プログラムID」と一致します。
これで案件名が分かり、案件のジャンルから出し先のカテゴリーまで決まります。
**CSVからリンクは作れません**（成果を紐づける `a8mat=` が入っていないため）。
リンクの取得は必ずA8の画面から行ってください。

A8の広告リンクを1件ずつコピーするのは骨が折れるので、収集の道具を2つ用意しています。

- `tools/a8-collect-bookmarklet.txt` … **ブックマークレット（推奨）**。
  ブックマークに登録し、A8の「広告リンク」ページで押すだけ。
  開発者ツールも `allow pasting` も要りません
- `tools/a8-collect.js` … 同じ処理を開発者ツールのコンソールで実行する版
- `tools/a8-collect-deep.js` … **一覧ページから、その先の広告リンクをたどって集める版**。
  「参加中プログラム」の一覧で実行すると、各案件のページを順に読んで
  コードを拾う（既定は1案件につき1件。1件ごとに0.4秒あけて読む）

どちらもコードには手を触れず、並べて `---` でつなぐだけです。

...
(function () {
  ...
  */
  ...
  /* 背景・光・角度は全記事で固定する。以前は部屋・角度・光を
     6×5×4通りから毎回選び、プロンプト側でも「過去の画像との連続性を
     完全に断ち切ること」と指示していた。一覧に並べたときに記事ごとに
     別のサイトのように見えるため、揃える側に倒した。
     変えるのは被写体（GM_SUBJECT）だけ。 */
  var GM_SETTING = {
    place: '白い壁の前に置かれた、オーク材の平らな天板の上',
    angle: '目線の高さから、やや斜め前方（三分の四）',
    light: '左手の窓から入る、曇り空のような柔らかい自然光。右からわずかな返し光'
  };

  /* 記事が扱っている商品そのものを描かせる。分野ごとの当たり障りない
     被写体だけを渡していたころは、記事と違う形の物が出ていた。
     商品名の売り文句と記号は落として、品名と型番だけを渡す。 */
  /* 品目の言葉。記事のタグ・説明文・題名から探して被写体に添える。
     商品名だけでは、モデルが何の品物か分からずまるで違う物を描く
     （「グンゼ AGW112」は靴下だが、白い電子機器が出てきた）。
     tools/make_images.py の KIND_WORDS と同じ内容。 */
  var GM_KINDS = ['靴下', 'ソックス', 'タイツ', 'ストッキング', 'インナー', '肌着', 'シャツ', 'Tシャツ', 'パンツ', 'ジャケット', 'コート', 'スニーカー', 'サンダル', 'ベルト', '財布', 'リュック', 'バッグ', '帽子', '手袋', 'マフラー', 'スーツケース', 'キャリーケース', 'ゴミ箱', 'ラック', '棚', '収納ボックス', 'デスク', 'チェア', '椅子', 'スツール', 'ベンチ', 'テーブル', '枕', 'マットレス', 'フットレスト', 'ハンガー', '扇風機', 'サーキュレーター', '加湿器', '除湿機', '空気清浄機', '掃除機', 'シーリングライト', 'パネルライト', '投光器', 'デスクライト', 'モニターライト', '電気ケトル', 'ケトル', '炊飯器', '電子レンジ', 'トースター', 'ミキサー', 'コーヒーメーカー', '圧力鍋', 'ホットプレート', 'アイロン', 'ヒーター', 'モニターアーム', 'モニター', 'ディスプレイ', 'マウス', 'キーボード', 'ドッキングステーション', 'ドック', 'USBハブ', 'ルーター', 'SSD', '外付けHDD', 'ノートパソコン', 'タブレット', '電子書籍リーダー', 'モバイルバッテリー', '充電器', 'ケーブル', 'スマートウォッチ', 'スマートリモコン', '防犯カメラ', 'webカメラ', 'イヤホン', 'ヘッドホン', 'スピーカー', 'スピーカーフォン', 'マイク', 'ピンマイク', 'カメラ', 'レンズ', '三脚', 'ジンバル', 'シェーバー', '電気シェーバー', 'カミソリ', '替刃', 'ドライヤー', 'ヘアアイロン', '脱毛器', '美顔器', '美容液', 'セラム', '化粧水', '乳液', 'マスカラ', 'アイブロウ', '口紅', '日焼け止め', 'シャンプー', '洗顔', 'フェイスマスク', 'シートマスク', 'シャワー', '水筒', 'タオル'];

  function gmKind(a) {
    /* 商品を1つ扱っていない記事（セール情報・特集）には品目が無い。
       説明文にたまたま出てきた言葉を拾うと、関係のない物を描く。 */
    if (!String(a.asin || '').trim()) return '';
    var srcs = [(a.tags || []).join(' '), a.description || '',
                a.excerpt || '', a.title || ''];
    var ws = GM_KINDS.slice().sort(function (x, y) { return y.length - x.length; });
    for (var i = 0; i < srcs.length; i++) {
      var low = String(srcs[i]).toLowerCase();
      for (var j = 0; j < ws.length; j++) {
        if (low.indexOf(ws[j].toLowerCase()) >= 0) return ws[j];
      }
    }
    return '';
  }

  function gmProduct(a) {
    var t = String(a.product_name || a.title || '');
    t = t.split('｜')[0].split('|')[0];
    t = t.replace(/[（(\[【][^）)\]】]*[）)\]】]/g, ' ');
    t = t.replace(/(の)?(口コミ|レビュー|評価|選び方|比較|仕様分析|徹底比較).*$/, '');
    /* 「◯◯の電気代と静音性」のような、記事の切り口までは被写体ではない */
    t = t.replace(/の(電気代|静音性|選び方|比較|違い|注意点|使い方|評判|実力|効果|設置条件|向き不向き).*$/, '');
    return t.replace(/\s+/g, ' ').trim();
  }

  function gmPromptFor(a) {
    var s = GM_SUBJECT[(a.category || '') + '/' + (a.sub || '')] ||
            ['a single unbranded consumer product', ''];
    var prod = gmProduct(a);
    var kind = gmKind(a);
    return [
      '以下の条件・テイストに厳格に従って、単独の画像を1枚作成してください。',
      '',
      '1. スタイルと質感（絶対条件）',
      '',
      ...
      '',
      '小道具・装飾・観葉植物は置かないこと。天板の上には被写体だけを置くこと。',
      '',
      '3. 背景と光（毎回まったく同じにすること・最重要）',
      '',
      'この画面で作る画像は、サイトの記事一覧に並びます。**1枚ごとに背景が変わると、記事ごとに別のサイトのように見えます。**',
      '',
      '背景・光・角度は下記のとおり毎回まったく同じにし、**変えてよいのは被写体だけ**とすること。',
      '',
      '場所：' + GM_SETTING.place,
      '角度：' + GM_SETTING.angle,
      '光：' + GM_SETTING.light,
      '色味：彩度は控えめ。影は柔らかく短く。壁は無地の白のまま（模様や色を付けないこと）',
      '',
      '4. 今回の被写体（この記事のために追記）',
      '',
      /* 商品名が分かるときは、それだけを渡す。分野ごとの被写体を
         添えると食い違うことがある（スピーカーフォンの記事に
         「ラベリアマイク」が付いていた）。 */
      /* 商品名と品物の種類を組にして渡す。商品名だけだと知らない型番で
         別の物を描き、種類だけだと分野の決め打ちで記事と食い違う。 */
      '撮影する物：' + (prod && kind ? prod + '（' + kind + '）' : (prod || s[0])),
      '被写体が画面の7割ほどを占め、三分割法でやや中心から外して置くこと。',
      '',
      '5. ロゴと商標（絶対条件）',
      '',
      '**商品の見た目は実物どおりに描くこと。** 形・大きさの釣り合い・色・素材・',
      'ボタンやポートの配置は、その商品が実際に持っているとおりにすること。',
      '',
      '**そのうえで、文字・数字・記号のたぐいは一切描かないこと。**',
      'ロゴ・ブランド名・商標・型番の印字や刻印はもちろん、ボタンの上の',
      'アイコンや絵記号も描かないこと。無地のボタンが正しく、何か描かれた',
      'ボタンは誤りとする。',
      '実物で印字がある面は、無地のまま（何も描かれていない状態）にすること。',
      '形を崩した文字や、似せた別の記号を置くこともしないこと。',
      '中途半端に入れるくらいなら、何も無いほうがよい。',
      '',
      '横長（16:9）で出力すること。'
    ].join('\n');
  }
  ...
  function articleRequest(a, prompt) {
    ...
    return [
      ...
      '・voices に stars（星の数）を入れない。voices は購入者レビューの傾向の'
        + '要約であって、特定の投稿の引用ではない。平均評価も個別の評価点も'
        + '確認できていないので、星を付ける根拠が無い。'
  ...
  var VAGUE_RIVAL = /一般的な(?:商品|製品|モデル|美容液|クリーム|タイプ|もの)|同クラス(?:の)?(?:製品|商品)|平均的な(?:商品|製品|モデル)|標準的な(?:商品|製品|モデル)|[（(]一般(?:的)?[）)]|一般タイプ|他社(?:の)?一般/g;
  ...
  function auditArticle(a) {
    ...
    /* 口コミの要約に星の数が付いていないか。voices は傾向の要約であって
       特定の投稿の引用ではないので、星を付けると確認していない評価点になる。 */
    (a.voices || []).forEach(function (v, i) {
      if (v && v.stars) {
        warns.push('voices[' + i + '] に星の数（' + v.stars + '）が付いている'
                   + '。口コミの要約に評価点は付けない');
      }
    });
  ...
  function applyGenerated(a, gen) {
    ...
    /* 生成AIが「根拠が無いので出さない」と判断して省いた rating と spec は、
       上書きしかしないと前の版の値が残ってしまう。省かれた＝出さないなので落とす。 */
    ['rating', 'spec'].forEach(function (k) {
      var v = gen[k];

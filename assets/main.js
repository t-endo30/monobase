...
/* ============================================================
   PCトップ：CATEGORIES／特集／検索の高さをそろえる
   ------------------------------------------------------------
   3カラムになる幅（1240px〜）だけ、左のCATEGORIES・右の検索タイルを、
   中央の特集の高さに合わせる。特集は写真つきの帯なので、これを基準に
   する（CATEGORIESの件数分だけ伸ばすと検索タイルが間延びしすぎるため）。
   CATEGORIESは中身が収まらなければ内側だけスクロールさせ、
   検索タイルは中身を縦中央に置く。
   ============================================================ */
(function () {
  'use strict';
  var cat = document.querySelector('.side-col > .side-nav');
  var feature = document.querySelector('.layout-main > .tb[data-tb="feature"]');
  var search = document.querySelector('.side-rank > .side-search');
  if (!feature || (!cat && !search)) return;
  var targets = [cat, search].filter(Boolean);

  var mq = window.matchMedia ? window.matchMedia('(min-width:1240px)') : null;

  function reset() {
    targets.forEach(function (el) { el.style.height = ''; });
  }

  function sync() {
    if (!mq || !mq.matches) { reset(); return; }
    reset();

...
{ad_note}      <p>広告・アフィリエイトの詳細な方針は<a href="{p}advertising.html">広告掲載について</a>に記載しています。</p>
    ...
    disclaimer = f'''      <h2>掲載情報の正確性について</h2>
      ...
      <p>当サイトの記事は、メーカーが公表している仕様と、販売ページ等で公開されている購入者レビューを突き合わせて整理・分析したものです。実機を用いた検証を行っていない記事について、当サイトが実際に使用したかのように記述することはありません。使用環境・体格・好みによって感じ方は異なり、効果や満足度を保証するものではありません。</p>
      <p>参照している購入者レビューは、販売ページに掲載されている一般利用者の意見であり、その正確性を当サイトが保証するものではありません。当サイトが記事内で示す評価は、公開情報にもとづく当サイト独自の見解であり、メーカー・販売店・Amazon等による評価ではありません。</p>
    ...
    about = f'''      <p>{e(NAME)} は、「{e(TAGLINE)}」をコンセプトに、暮らしと作業を快適にするアイテムを紹介するメディアです。何を基準に記事を作っているかは<a href="{p}editorial-policy.html">記事作成方針</a>のページにまとめています。</p>
      ...
      <h2>口コミの扱い</h2>
      <p>購入者レビューは<strong>参考情報</strong>として扱い、メーカー公表の仕様や当サイトの分析とは区別して書きます。当サイトが実機で検証していない商品について、使用体験として記述することはありません。存在しない口コミを作ることはせず、確認できていない件数や割合を数字で書くこともしません。</p>
...
{policy_items}      </div>
      ...
      <p>記事の中では、<strong>「メーカー公表の仕様」「購入者レビューから読み取れること」「当サイトの分析」</strong>の3つを、読者が読み分けられる書き方で分けています。分析や見通しを事実として断定することはしません。</p>
      <p>メーカー公式サイトが存在しない商品や、公式ページから型番を特定できない商品もあります。その場合は仕様表や評価点を無理に埋めず、確認できた範囲だけで記事を構成します。</p>

      <h2>記事で行わないこと</h2>
      <ul>
        <li>確認できない情報を、推測で補って書かない</li>
        <li>公式仕様と購入者レビューを混ぜて書かない</li>
...
{ad_kind}{asp_kind}      </dl>
      ...
      <h2>広告と、記事の内容の関係</h2>
      <p>広告収益の有無や紹介料率の高さによって、商品の評価内容や掲載可否を変えることはありません。紹介料が高い商品を優先して取り上げることも、低い商品の欠点を強調することもしません。評価は公開されている仕様と購入者レビューにもとづいて行い、その考え方は<a href="{p}editorial-policy.html">記事作成方針</a>に記載しています。</p>
